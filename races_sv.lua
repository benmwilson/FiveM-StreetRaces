    ...
    while true do
        ...
        for index, race in pairs(races) do
            ...
            if (time > race.startTime) and (#players == 0) then
                ...
                TriggerClientEvent("GRace:removeRace_cl", -1, index)
...
RegisterNetEvent("GRace:createRace_sv")
AddEventHandler("GRace:createRace_sv", function(amount, startDelay, startCoords, checkpoints, finishTimeout)
    ...
    local index = #races
    TriggerClientEvent("GRace:createRace_cl", -1, index, amount, startDelay, startCoords, checkpoints)
end)

-- Server event for canceling a race
RegisterNetEvent("GRace:cancelRace_sv")
AddEventHandler("GRace:cancelRace_sv", function()
...
RegisterNetEvent("GRace:joinRace_sv")
AddEventHandler("GRace:joinRace_sv", function(index)
    ...
    if playerMoney >= amount then
        ...
        TriggerClientEvent("GRace:joinedRace_cl", source, index)
    else
...
RegisterNetEvent("GRace:leaveRace_sv")
AddEventHandler("GRace:leaveRace_sv", function(index)
...
RegisterNetEvent("GRace:finishedRace_sv")
AddEventHandler("GRace:finishedRace_sv", function(index, time)
...
RegisterNetEvent("GRace:saveRace_sv")
AddEventHandler("GRace:saveRace_sv", function(name, checkpoints)
...
RegisterNetEvent("GRace:deleteRace_sv")
AddEventHandler("GRace:deleteRace_sv", function(name)
...
RegisterNetEvent("GRace:listRaces_sv")
AddEventHandler("GRace:listRaces_sv", function()
...
RegisterNetEvent("GRace:loadRace_sv")
AddEventHandler("GRace:loadRace_sv", function(name)
    ...
    if race ~= nil then
        ...
        TriggerClientEvent("GRace:loadRace_cl", source, race)
