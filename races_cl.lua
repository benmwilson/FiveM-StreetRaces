...
RegisterCommand("race", function(source, args)
    if args[1] == "clear" or args[1] == "leave" then
        ...
        if raceStatus.state == RACE_STATE_JOINED or raceStatus.state == RACE_STATE_RACING then
            cleanupRace()
            TriggerServerEvent('GreysonRacing:leaveRace_sv', raceStatus.index)
        end
    ...
    elseif args[1] == "save" then
        ...
        if name ~= nil and #recordedCheckpoints > 0 then
            ...
            TriggerServerEvent('GreysonRacing:saveRace_sv', name, recordedCheckpoints)
        end
    elseif args[1] == "delete" then
        -- Check name was provided and send event to server to delete saved race
        local name = args[2]
        if name ~= nil then
            TriggerServerEvent('GreysonRacing:deleteRace_sv', name)
        end
    elseif args[1] == "list" then
        -- Send event to server to list saved races
        TriggerServerEvent('GreysonRacing:listRaces_sv')
        notifyPlayer(source, "showing list")
    elseif args[1] == "load" then
        -- Check name was provided and send event to server to load saved race
        local name = args[2]
        if name ~= nil then
            TriggerServerEvent('GreysonRacing:loadRace_sv', name)
            utils.ui.showNotification('RACE LOADED')
        end
    elseif args[1] == "start" then
        ...
        if amount then
            ...
            if #recordedCheckpoints > 0 then
                ...
                TriggerServerEvent('GreysonRacing:createRace_sv', amount, startDelay, startCoords, recordedCheckpoints)
    ...
    elseif args[1] == "cancel" then
        ...
        TriggerServerEvent('GreysonRacing:cancelRace_sv')
    else
        return
    end
end)
...
RegisterNetEvent("GreysonRacing:createRace_cl")
AddEventHandler("GreysonRacing:createRace_cl", function(index, amount, startDelay, startCoords, checkpoints)
...
RegisterNetEvent("GreysonRacing:loadRace_cl")
AddEventHandler("GreysonRacing:loadRace_cl", function(checkpoints)
...
RegisterNetEvent("GreysonRacing:joinedRace_cl")
AddEventHandler("GreysonRacing:joinedRace_cl", function(index)
...
RegisterNetEvent("GreysonRacing:removeRace_cl")
AddEventHandler("GreysonRacing:removeRace_cl", function(index)
                ...
                if raceStatus.checkpoint == 0 then
                    ...
                    if GetDistanceBetweenCoords(position.x, position.y, position.z, checkpoint.coords.x, checkpoint.coords.y, 0, false) < config_cl.checkpointProximity then
                        ...
                        if raceStatus.checkpoint == #(race.checkpoints) then
                            ...
                            TriggerServerEvent('GreysonRacing:finishedRace_sv', raceStatus.index, currentTime)
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < config_cl.joinProximity and currentTime < race.startTime then
                        ...
                        if IsControlJustReleased(1, config_cl.joinKeybind) then
                            TriggerServerEvent('GreysonRacing:joinRace_sv', index)
                            break
                        end
                    end
                end
